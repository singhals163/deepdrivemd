...
class CVAETrainApplication(Application):
    ...
    def run_active(self, model_ref: str, input_data: CVAETrainInput) -> CVAETrainOutput:
        """State 2: Execute training using the cached GPU model and sync to disk."""
        ...
        # Locate the checkpoint generated by trainer.fit
        checkpoint_dir = model_dir / "checkpoints"
        model_weight_path = natsorted(list(checkpoint_dir.glob("*.pt")))[-1]
        
        # RESTORED: Re-route the path to the persistent_dir where it will be synced
        ...
        # RESTORED: Output YAML for completeness
        output_data.dump_yaml(self.workdir / "output.yaml")
        
        # RESTORED: Sync the local workdir to the shared persistent_dir so Inference can read it
